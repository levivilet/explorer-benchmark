...
export const labels = { lvce: 'LVCE explorer-view', pierre: 'Pierre / trees.software' }
...
export function renderChart(groups) {
  ...
  return `<svg viewBox="0 0 800 190" role="img" aria-label="Retained JavaScript heap medians and ranges in MiB; failed loads have no bar"><title>Loaded tree retained JavaScript heap; lower uses less</title>${Object.entries(groups).map(([key, group], i) => {
    const y = 35 + i * 85
    const label = `<text x="0" y="${y + 22}" fill="currentColor" font-size="16">${labels[key]}</text>`
