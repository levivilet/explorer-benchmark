  ...
  for (const trialInfo of order) {
    ...
    try {
      ...
      for (const phase of ['empty', 'loaded']) {
        ...
        const state = await page.evaluate((phase) => window.benchmark.load(phase), phase)
        ...
        if (phase === 'loaded') {
          ...
          await page.evaluate(() => window.benchmark.scroll(0))
...
if (report.trials.some(({ status }) => status !== 'passed')) process.exitCode = 1
