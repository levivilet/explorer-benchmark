...
import { DEFAULT_FILES } from './fixture.ts'
import { implementations } from './implementations.ts'
import { aggregate } from './report.ts'
import type { BenchmarkReport, Feasibility, Implementation, Protocol } from './types.ts'

type JsonRecord = Record<string, any>
const readJson = async (path: string): Promise<JsonRecord> => JSON.parse(await readFile(path, 'utf8'))
const writeJson = async (path: string, value: unknown): Promise<void> => { await writeFile(path, JSON.stringify(value, null, 2) + '\n') }

export async function mergeResults(source = 'shards', destination = 'results', files: number[] = DEFAULT_FILES.split(',').map(Number)): Promise<void> {
  const shards: Array<{ directory: string; manifest: JsonRecord; implementation: Implementation }> = []
  for (const name of await readdir(source)) {
    ...
    assert.deepEqual(manifest.workloads.map(({ count }: { count: number }) => count), manifest.files)
    assert(manifest.files.every((count: number) => files.includes(count)), 'Unexpected workload')
    if (shards.length) {
      for (const key of ['commit', 'runUrl', 'protocol']) assert.deepEqual(manifest[key], shards[0]!.manifest[key], `Mismatched shard ${key}`)
    }
    shards.push({ directory, manifest, implementation: manifest.implementations[0] as Implementation })
  }
  assert(shards.length, 'No benchmark shards')
  await mkdir(destination, { recursive: true })
  const manifest: JsonRecord = { ...shards[0]!.manifest, files, implementations, status: 'running', workloads: [] as Array<{ count: number; status: string }> }
  ...
  for (const count of files) {
    ...
    const unavailable: Partial<Record<Implementation, Feasibility>> = {}
    const provenance = []
    ...
    for (const implementation of implementations) {
      ...
      const workload = shard.manifest.workloads.find((workload: { count: number }) => workload.count === count)
      ...
      if (workload.status === 'infeasible') {
        ...
        const feasibility = await readJson(`${input}/feasibility.json`) as unknown as Feasibility
      ...
      const report = await readJson(`${input}/results.json`) as unknown as BenchmarkReport
      ...
      for (const key of ['repeats', 'samples', 'seed'] as const) assert.equal(report.protocol[key], (manifest.protocol as Protocol)[key])
      aggregate(report)
      if (reports.length) {
        for (const key of ['schemaVersion', 'mode', 'fixture', 'sources', 'packageLockSha256'] as const) assert.deepEqual(report[key], reports[0]![key], `Mismatched ${key}`)
        const sharedProtocol = ({ implementations, order, ...protocol }: Protocol) => protocol
...
if (process.argv[1] && import.meta.url === pathToFileURL(process.argv[1]).href) await mergeResults(process.argv[2], process.argv[3])
